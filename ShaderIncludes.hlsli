#ifndef __GGP_SHADER_INCLUDES__ // Each .hlsli needs a uniqueidentifier!
...
// Struct representing the data we're sending down the pipeline (and is received by the pixel shader!)
...
struct VertexToPixel
{
	...
	float3 tangent			: TANGENT;		//Tangent (for normal map)
};
...
struct Light
{
	...
	float3 Direction;		//directional/spot lights have direction
	float Range;			// Point/Spot have max range
	float3 Position;		//Point/spot light position in space
	float Intensity;		//All lights - light intensity
	float3 Color;			//All lights - color
	float SpotFalloff;		//Spot lights - value to restrict cone
	float3 Padding;			//Purposefully padding to hit 16-byte boundary
};
